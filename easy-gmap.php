...
Plugin URI: https://wordpress.org/plugins/sdevs-wc-gmap
Description: Embed a Google Map on your site to show your store location.
Version: 1.0.0
Author: SpringDevs
Author URI: https://springdevs.com/
License: GPLv2
License URI: https://www.gnu.org/licenses/gpl-2.0.html
Text Domain: sdevs_gmap
...
final class Sdevs_Easy_Gmap
{
    ...
    {
        $installer = new springdevs\Gmap\Installer();
        $installer->run();
    }
    ...
    public function includes()
    {
        if ($this->is_request('admin')) {
            $this->container['admin'] = new \springdevs\Gmap\Admin();
        }

        if ($this->is_request('frontend')) {
            $this->container['frontend'] = new \springdevs\Gmap\Frontend();
        }
    ...
    {
        if ($this->is_request('ajax')) {
            $this->container['ajax'] =  new \springdevs\Gmap\Ajax();
        }
        $this->container['assets'] = new \springdevs\Gmap\Assets();
    }
    ...
    {
        load_plugin_textdomain('sdevs_gmap', false, dirname(plugin_basename(__FILE__)) . '/languages/');
    }
